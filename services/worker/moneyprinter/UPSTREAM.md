...
- `services/worker/worker.py` configures MoneyPrinterTurbo from Cloud Run job environment variables, builds `app.models.schema.VideoParams`, and calls `app.services.task.start(..., stop_at="video")`.
- `services/worker/Dockerfile` installs MoneyPrinterTurbo dependencies, installs the MoneyPrint worker dependencies, copies the full vendored tree, and runs `python /app/worker.py`.
...
4. Confirm `services/worker/worker.py` still maps MoneyPrint job fields to valid `VideoParams` fields.
